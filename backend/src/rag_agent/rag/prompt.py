...
class RAGPrompt:
    ...
    def _format_history(self, history: list) -> str:
        ...
        result = "\n".join(formatted)
        logger.info(f"格式化历史: {len(history)} 条 -> {result[:200]}")
        return result

    async def get_rag_jdandlaws_template(self, history: list = None):
        logger.info("开始创建 rag_jdandlaws 提示词模板")
        history_str = self._format_history(history) if history else "（无历史记录）"
        history_section = f"\n【对话历史】（请务必参考以下历史对话来回答问题）\n{history_str}\n"
...
一、核心规则
    ...
    4. 多轮对话：参考对话历史，保持上下文连贯性，理解代词指代（如"这个"、"它"、"刚才"、"之前"等）。重要：如果用户问题涉及历史对话中的信息，必须结合历史来回答！
...
"""
    ...
    def get_jd_template(self, history: list = None):
